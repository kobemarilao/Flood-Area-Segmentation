...
if not os.path.exists("road.pth"):
    gdown.download("https://drive.google.com/uc?id=1qeQAb9-nrUEr7XGCa2DXERmIYHnJeRuZ", "road.pth", quiet=False)

if not os.path.exists("flood.pth"):
    gdown.download("https://drive.google.com/uc?id=1EuzZeEwa8Gd1lHGyL13n7uOVp0rHiNoS", "flood.pth", quiet=False)

@st.cache_resource
def load_model():
    model = torch.load("road.pth", map_location=torch.device('cpu'))
    model.eval()
    return model
...
def load_model2():
    ...
    model2.load_state_dict(torch.load("flood.pth", map_location=torch.device('cpu')))
    model2.to('cpu')
    model2.eval()
    return model2
...
if menu_selection == 'Home':
    ...
    **What We Do**
    ...
    with col2:
        ...
        st.markdown('<p style="text-align: left; margin-bottom: 0;">Christian Kobe A. Marilao</p>', unsafe_allow_html=True)
        st.markdown('<p style="text-align: left; margin-top: 0;">ckamarilao@usep.edu.ph</p>', unsafe_allow_html=True)


    st.write("\n")
    st.markdown("""
    The dashboard was developed by a dedicated team of 2, namely, Kean David and  Christian Kobe Marilao. 
